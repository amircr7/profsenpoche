...
app.controller('MainCtrl', ['$scope', '$ionicSideMenuDelegate', '$ionicHistory','$http', function($scope, $ionicSideMenuDelegate, $ionicHistory,$http) {
	...
	$http.get('http://94.23.13.72/planeteprofs/wp-admin/admin-ajax.php?action=app_envoie_true_si_userconnect')
...
app.controller('MessageCtrl', ['$scope', 'Messages', '$ionicScrollDelegate','$http', function($scope, Messages, $ionicScrollDelegate,$http){
	...
	$http.get('http://94.23.13.72/planeteprofs/wp-admin/admin-ajax.php?action=app_get_currentuser')
		.success(function(data,headers,config)
		{
			$scope.currentusername=data.substring(0, data.length - 1).replace('\n', '').replace('\n', '');




			/*alert('test : '+$scope.currentusername.replace('\n', '').replace('\n', ''));*/
		})
		.error(function(data,headers,config)
		{
			console.log('erreur !')
	...
	/*$http({
		method: 'GET',
		url: 'http://94.23.13.72/planeteprofs/wp-admin/admin-ajax.php?action=app_get_currentuser'
	}).then(function successCallback(response) {
		$scope.currentuserID=response.data;

		alert('currentuserid : '+$scope.currentuserID);
	...
	$scope.addMesage = function(){
		...
		$scope.messages = $scope.messages.concat($scope.datamessage);
		$scope.datamessage = "";
		$ionicScrollDelegate.scrollBottom();
